...
const { printResults } = require('../src/reporter');
const { loadConfig, severityRank } = require('../src/config');

async function main() {
	const urlArg = process.argv[2];
	if (!urlArg) {
		console.error('Usage: web-scanner <url> [--json] [--zap] [--config path] [--min-sev Sev]');
		process.exit(1);
	}
